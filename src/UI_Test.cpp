    ...
    while (!glfwWindowShouldClose(window))
        {
            ...
            // Tip: if we don't call ImGui::Begin()/ImGui::End() the widgets automatically appears
            // in a window called "Debug".
            {
                static float f = 0.0f;
                ...
                if (ImGui::Button("Button"))  // Buttons return true when clicked (NB: most widgets
            ...
            // 2. Show another simple window. In most cases you will use an explicit Begin/End pair
            ...
            if (show_demo_window)
                {
                    ImGui::SetNextWindowPos(
                        ImVec2(650, 20), ImGuiCond_FirstUseEver);  // Normally user code doesn't
                                                                   // need/want to call this because
                                                                   // positions are saved in .ini
                                                                   // file anyway. Here we just want
                                                                   // to make the demo initial state
                                                                   // a bit more friendly!
